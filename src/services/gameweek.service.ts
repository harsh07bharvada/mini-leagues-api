...
export default class GameweekService {
   ...
   * @returns
   */
  static async getCurrentGameweekDetails(): Promise<IGameweek> {
    ...
    try {
      ...
      const currentEventStatus: any = await HTTPService.get(
        currentEventStatusURL
      )
      ...
      if (
        !currentEventStatus ||
        !currentEventStatus[KEYS.STATUS] ||
        !currentEventStatus[KEYS.STATUS].length
      ) {
        throw Error(ERROR_MSGS.GET_EVENT_STATUS_FAILED)
      }

      //ASSIGN CURRENT GAMEWEEK ID
      gameweekDetails.gameweekID =
        currentEventStatus[KEYS.STATUS][0][KEYS.EVENT]

      //GET FIXTURE DETAILS FOR CURRENT GAMEWEEK
      const currentEventFixturesURL: string =
        URLS.GET_CURRENT_EVENT_DETAILS + gameweekDetails.gameweekID
      const currentEventFixtures: any = await HTTPService.get(
        currentEventFixturesURL
      )

      if (!currentEventFixtures)
        throw Error(ERROR_MSGS.GET_EVENT_FIXTURES_FAILED)

      gameweekDetails.fixtures = currentEventFixtures

      gameweekDetails.firstKickoffTime =
        currentEventFixtures[0][KEYS.KICK_OFF_TIME]
      gameweekDetails.lastKickffTime =
        currentEventFixtures[currentEventFixtures.length - 1][
          KEYS.KICK_OFF_TIME
        ]
